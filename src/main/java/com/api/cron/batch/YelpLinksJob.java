...
import com.api.cron.batch.jobs.yelp.LoadLinkByCityStateService;

/**
 * 
 * Create links in the DB and then runs the yelp scrapper job to read businesses from each link.
 * This job can pick up where it left off
 * 
 * @author khimung
 *
 */
public class YelpLinksJob implements BatchJob {
	private final static Logger logger = Logger.getLogger(YelpLinksJob.class);
	...
	public YelpLinksJob(String[] args) {
		this.args = args;
	}
	...
	public void execute() throws CronJobException {
		...
		try {
			...
			Integer lastProcessedKey = storeJdbcTemplate.queryForObject("select max(id) from yelp_links ", Integer.class);
			
			if(lastProcessedKey == null){
				lastProcessedKey = 0;
			}
			
			LoadLinkByCityStateService loadLinkByCityStateService = (LoadLinkByCityStateService) context.getBean("yelpLoadLinkByCityService");
			loadLinkByCityStateService.load(city, state, categories);
			
			JobState jobState = (JobState) context.getBean("jobState");
			
			JobLauncher jobLauncher = (JobLauncher) context.getBean("jobLauncher");
			Job job = (Job) context.getBean("linksYelpJob");
			

			JobParametersBuilder builder = new JobParametersBuilder();
			builder.addString("quota", jobState.getSuccess() + "");
			builder.addString("lastProcessedKey", lastProcessedKey.toString());
			builder.addString("jobName", "linksYelpJob");
